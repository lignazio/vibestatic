...
final class ModulesTest extends TestCase {
    ...
    public function setUp() : void {
        ...
        if ( ! defined( 'VIBESTATIC_VERSION' ) ) {
            define( 'VIBESTATIC_VERSION', '8.1.0' );
        }
    }
     ...
     *
     * **There is no baseline now**, which is the strongest form of the same
     * guarantee: the 155 entries it held were fixed rather than carried. The
     * test stays because a baseline is one command away, and the day somebody
     * regenerates one is the day this needs to be here already.
     */
    public function testTheBaselineDoesNotHideBrokenHookCallbacks() : void {
        $path = VIBESTATIC_PATH . 'phpstan-baseline.neon';

        if ( ! is_readable( $path ) ) {
            $this->assertFalse(
                file_exists( $path ),
                'A baseline that cannot be read is worse than none.'
            );
