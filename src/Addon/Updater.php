 ...
 * repository** — right when the repository holds one plugin, and wrong the
 * moment it holds ten. Asked that question, a site with BunnyCDN installed
 * would be offered whatever came out last, Azure included, and WordPress would
 * install it over the top.
 *
 * So this one lists `/releases` and takes the newest whose tag begins with the
 * add-on's own prefix — `bunnycdn-v1.0.1`, never `azure-v2.0.0`. It is the
 * point at which a monorepo breaks silently, so there is a test for exactly
 * that.
 *
 * One request serves every add-on installed: the release list is fetched once
 * per repository and cached, and each add-on filters the same list. Ten add-ons
 * against GitHub's sixty-requests-an-hour anonymous limit would otherwise be
 * ten requests every time WordPress checks for updates.
 *
...
class Updater {
     ...
     * the repository so ten installed add-ons share a single request.
     *
     * @return list<mixed>
     */
